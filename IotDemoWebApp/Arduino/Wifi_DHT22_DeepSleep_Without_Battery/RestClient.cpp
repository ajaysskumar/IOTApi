...
void RestClient::setConnectionTimeout(int timeoutPeriod){
  connectionTimeout = timeoutPeriod;
  HTTP_DEBUG_PRINT("timeout set to "+String(connectionTimeout)+"\n");
}
...
int RestClient::readResponse(String* response) {
  ...
  while (client.connected()) {
	...
	if(!clientAvailable)
	{
	HTTP_DEBUG_PRINT("Waiting for reply : "+String(sleepCounter)+"\n");
	delay(1000);
	sleepCounter++;
	}
	if(sleepCounter==connectionTimeout && !clientAvailable)
	{
		HTTP_DEBUG_PRINT("Stopping client\n");
		client.stop();
	}
